...
def installed_cuda_version():
    ...
    if cuda_home is None:
        raise MissingCUDAException(
            "CUDA_HOME does not exist, unable to compile CUDA op(s)")
    # Ensure there is no version mismatch between torch and nvcc compiler
...
def get_default_compute_capabilities():
    ...
    if torch.utils.cpp_extension.CUDA_HOME is not None \
        ...
        if installed_cuda_version()[0] == 11 \
            ...
            # Special treatment of CUDA 11.0: compute_86 not supported.
            compute_caps += ";8.0"
        else:
...
# list compatible minor CUDA versions, so that
# for example pytorch built with cuda-11.0 can be used to
# build flextrain and system-wide installed cuda 11.2
...
class OpBuilder(ABC):
    ...
    def absolute_name(self):
        ...
        e.g., flextrain.ops.adam.cpu_adam will be installed as
        something like: flextrain/ops/adam/cpu_adam.so
        '''
        pass
    ...
    def sources(self):
        '''
        Returns list of source files for your op, relative to flextrain/ops
        '''
        pass
    ...
    def is_rocm_pytorch():
        ...
        try:
            ...
            if TORCH_MAJOR > 1 or (TORCH_MAJOR == 1 and TORCH_MINOR >= 5):
                _is_rocm_pytorch = hasattr(torch.version, 'hip') and \
    ...
    def is_sycl_enabled():
        ...
        try:
            subprocess.run(["c2s", "--version"], capture_output=True)
    ...
    def include_paths(self):
        '''
        Returns list of include paths, relative to flextrain/ops
        '''
        return []

    def nvcc_args(self):
        '''
        Returns optional list of compiler flags to
    ...
    def has_function(self, funcname, libraries, verbose=False):
        ...
        This is used as a smoke test to check whether a certain library is
        available. As a test, this creates a simple C program that calls
        the specified function, and then distutils is used to compile that
        program and link it with the specified libraries. Returns True
        if both the compile and link are successful, False otherwise.
        '''
        ...
        try:
            ...
            # Configure compiler & linker to build according to Python install.
            ...
            # Redirect stderr file descriptor to a file
            # to silence compile/link warnings.
            if not verbose:
                filestderr = open(os.path.join(tempdir, 'stderr.txt'), 'w')
                oldstderr = os.dup(sys.stderr.fileno())
                os.dup2(filestderr.fileno(), sys.stderr.fileno())

            # Workaround for behavior in
            ...
            # Restore stderr file descriptor, close the stderr redirect file.
            if oldstderr is not None:
                os.dup2(oldstderr, sys.stderr.fileno())
            if filestderr is not None:
                filestderr.close()

            # Delete the temp dir holding the test program and stderr files.
    ...
    def _backup_cpuinfo(self):
        # Construct cpu_info dict from lscpu
        # that is similar to what py-cpuinfo provides
    ...
    def command_exists(self, cmd):
        ...
        if not valid and len(cmds) > 1:
            print(
                f"{WARNING} {self.name} requires one of "
                f"the following commands '{cmds}', but it does not exist!")
    ...
    def flextrain_src_path(self, code_path):
    ...
    def builder(self):
        ...
        include_dirs = [
            os.path.abspath(x) for x in self.strip_empty_entries(
                self.include_paths()
            )
        ]
        return CppExtension(
    ...
    def jit_load(self, verbose=True):
        ...
        # Torch will try and apply whatever CCs are in the arch list
        # at compile time, we have already set the intended targets ourselves
        # we know that will be needed at runtime.
        # This prevents CC collisions such as multiple __half
        ...
        # Reset arch list so we are not silently
        # removing it for other possible use cases
        if torch_arch_list:
...
class CUDAOpBuilder(OpBuilder):
    ...
    def compute_capability_args(self, cross_compile_archs=None):
        ...
        3. Under `jit_mode` \
            compute capabilities of all visible cards will be used plus PTX
        ...
        ccs = []
        if self.jit_mode:
            # Compile for underlying architectures
            ...
            if cross_compile_archs_env is not None:
                if cross_compile_archs is not None:
                    print(
                        f"{WARNING} env var "
                        f"`TORCH_CUDA_ARCH_LIST={cross_compile_archs_env}` "
                        f"overrides "
                        f"`cross_compile_archs={cross_compile_archs}`"
                    )
    ...
    def filter_ccs(self, ccs: List[str]):
        """
        Prune any compute capabilities that are
        not compatible with the builder.
        Should log which CCs have been pruned.
        """
        return ccs

    def version_dependent_macros(self):
        # Fix from apex that might be relevant for us as well,
        # related to https://github.com/NVIDIA/apex/issues/456
        version_ge_1_1 = []
    ...
    def builder(self):
        ...
        if self.build_for_cpu:
            from torch.utils.cpp_extension \
                import CppExtension as ExtensionBuilder
        else:
            from torch.utils.cpp_extension \
                import CUDAExtension as ExtensionBuilder
        include_dirs = [
            os.path.abspath(x) for x in self.strip_empty_entries(
                self.include_paths()
            )
